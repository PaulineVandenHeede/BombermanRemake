...
void MainGS(point VS_DATA vertex[1], inout TriangleStream<GS_DATA> triStream)
{
	...
	float2 charSizeTex = charSize;
	charSizeTex.x /= gTextureSize.x;
	charSizeTex.y /= gTextureSize.y;
...
float4 MainPS(GS_DATA input) : SV_TARGET{
	...
	float value = gSpriteTexture.Sample(samPoint, input.TexCoord)[input.Channel];

	return input.Color * value; //TEMP
}
