...
void ShadowMapRenderer::Initialize()
{
	...
	m_pShadowMapGenerator->Initialize(m_GameContext.d3dContext, m_pShadowMapGenerator->GetMaterialId());
...
void ShadowMapRenderer::UpdateMeshFilter(const SceneContext& sceneContext, MeshFilter* pMeshFilter) const
{
	...
	//pMeshFilter->BuildIndexBuffer(sceneContext);
...
void ShadowMapRenderer::Begin(const SceneContext& sceneContext)
{
	...
	//		*focusPosition: Calculate using the Directional Light position and direction
	//- Use the Projection & View Matrix to calculate the ViewProjection of this Light, store in m_LightVP
	const XMMATRIX projectionMatrix = DirectX::XMMatrixOrthographicLH(200.f * sceneContext.aspectRatio, 200.f, 0.1f, 500.f);
	const Light light = sceneContext.pLights->GetDirectionalLight();
	const XMVECTOR position = DirectX::XMLoadFloat4(&light.position);
	XMVECTOR direction = DirectX::XMLoadFloat4(&light.direction);
	XMVECTOR target = DirectX::XMVectorAdd(position, direction);
	const XMMATRIX viewMatrix = DirectX::XMMatrixLookAtLH(position, target, { 0.f, 1.f, 0.f });

	const XMMATRIX viewProjectionMatrix = DirectX::XMMatrixMultiply(viewMatrix, projectionMatrix);
	DirectX::XMStoreFloat4x4(&m_LightVP, viewProjectionMatrix);
...
void ShadowMapRenderer::DrawMesh(const SceneContext& sceneContext, MeshFilter* pMeshFilter, 
	const XMFLOAT4X4& meshWorld, const std::vector<XMFLOAT4X4>& meshBones)
{
	...
	const ShadowGeneratorType type = pMeshFilter->HasAnimations() ? ShadowGeneratorType::Skinned : ShadowGeneratorType::Static;
	const MaterialTechniqueContext technique = m_GeneratorTechniqueContexts[(int)type];

	m_pShadowMapGenerator->SetVariable_Matrix(L"gWorld", meshWorld);
	if(type == ShadowGeneratorType::Skinned)
	{
		m_pShadowMapGenerator->SetVariable_MatrixArray(L"gBones", *(meshBones.data()->m), (UINT)meshBones.size());
	}
	...
	for (const auto& subMesh : pMeshFilter->GetMeshes())
	{
		...
		//set index buffer
		pDeviceContext->IASetIndexBuffer(pMeshFilter->GetIndexBuffer(subMesh.id), DXGI_FORMAT_R32_UINT, 0);

		//set correct technique
		m_pShadowMapGenerator->SetTechnique((int)type);

		//draw call
		ID3DX11EffectTechnique* tech = m_pShadowMapGenerator->GetTechniqueContext().pTechnique;
...
void ShadowMapRenderer::End(const SceneContext&) const
{
	...
	//Now we can reset the Main Game Rendertarget back to the original RenderTarget,
	//this also Unbinds the ShadowMapRenderTarget as RTV from the Pipeline, and can safely use it as a ShaderResourceView after this point.
