...
void SpriteComponent::Draw(const SceneContext& sceneContext)
{
	...
	DirectX::XMFLOAT2 pos{ pTransform->GetWorldPosition().x , pTransform->GetWorldPosition().y };
	DirectX::XMFLOAT2 scale{ pTransform->GetScale().x, pTransform->GetScale().y };
	DirectX::XMFLOAT3 rot = MathHelper::QuaternionToEuler(pTransform->GetRotation());

	SpriteRenderer::Get()->AppendSprite(m_pTexture, pos, m_Color, m_Pivot, scale, rot.z, pTransform->GetPosition().z);
