...
#pragma region POST-PROCESSING PASS
	...
	if (m_PostProcessingMaterials.size() > 0)
	{
		...
		RenderTarget* initRenderTarget = m_pGame->GetRenderTarget();
		RenderTarget* previousRenderTarget = initRenderTarget;
		...
		size_t size = m_PostProcessingMaterials.size();
		for(size_t i = 0; i < size; ++i)
		{
			if (!m_PostProcessingMaterials[i]->IsEnabled())
				continue;

			m_PostProcessingMaterials[i]->Draw(m_SceneContext, previousRenderTarget);
			previousRenderTarget = m_PostProcessingMaterials[i]->GetOutput();
		}
		...
		if (previousRenderTarget != initRenderTarget)
		{
			m_pGame->SetRenderTarget(nullptr);
			SpriteRenderer::Get()->DrawImmediate(m_SceneContext.d3dContext, previousRenderTarget->GetColorShaderResourceView(),XMFLOAT2{ 0.f, 0.f });
		}
		//Done!
	}
#pragma endregion
